...
| `maps/glissieres.js` | Glissières au bord de toutes les rues : lames visibles (tirées du ruban de chaussée) + bande de retenue `glissiereAt` lue par la berline comme un mur ; rien aux carrefours, devant le skatepark ni dans les façades Depuis le 25/09/2026 : lames et bande de retenue aussi sur les tabliers de pont (tableau `ponts_axes` de build_village, `[n, largeur, x, y, z, …]` par tablier), et `creerAdherence` marque les tabliers comme chaussée. 26/09 : sur les ponts la lame est **contre le parapet** (w/2 − 0,36) et la bande commence 50 cm derrière ; partout, une cellule dont le centre est sur la chaussée n'est jamais marquée ; la trottinette est retenue aussi ; le pilote répond **en douceur** aux glissières (`reglagesAssistance.*Glissiere`, 95 % de la vitesse gardée) |
| `maps/fleches-sol.js` | Flèches jaunes peintes au sol le long d'un tracé (26/09/2026) — remplacent la bande verte de la course ; un seul maillage instancié |
| `maps/course-boucle.js` | La boucle de course de Poilhes City (26/09/2026) : `maps/<village>/boucle.json` (calculé par `scripts/poilhes/boucle_village.py` sur les rues du jeu), flèches, damier, 3-2-1, 2 tours, classement local |
| `maps/poilhes-enseignes.js` | Enseignes des commerces (26/09/2026) : bandeau de façade + enseigne drapeau, panneau sur pied faute de façade |
| `maps/survol-rafale.js` | Un Rafale passe au-dessus du village toutes les 40–85 s, son synthétisé (Poilhes City seulement) |
| `maps/poilhes-figurants.js` | Figurants de Poilhes City (26/09/2026, `assets/fun/`) : cycliste qui tourne sur la boucle, policier au départ, stégosaure au skatepark, Carole et les mafieux devant l'Ostal Louis |
| `maps/poilhes-helico.js` | L'hélicoptère d'Arnaud (gunship Meshy, `assets/fun/helicoptere.glb`) : orbite au-dessus de la course, et mode **Hélico** pilotable ; rotor découpé au chargement |
| `maps/volant.js` | Volant (réglage guidé, T150) et manette classique PS4/Xbox en voiture ; `volant-enregistreur.html` pour le diagnostic |
| `maps/musique.js` | La playlist du jeu (10 « Soul Run ») pour les Mondes et `poilhes.html`, bouton ♪, préférence partagée avec Poilhes City |
| `maps/multijoueur.js` + `multiplayer/village.py` | Multijoueur de Poilhes City (26/09/2026) : salon de village par pseudo (8 joueurs), avatars, course à plusieurs, classement en ligne ; actif seulement si le serveur Python répond |
| `maps/quad.js` | Le quad (25/09/2026) : quad Meshy seul + pilote riggé assis (mains au guidon, tête qui suit le volant), quatre roues découpées par essieu (les avant braquent), éclairage complet posé d'après la boîte englobante (phares + faisceaux, barre LED, feux arrière/stop, recul, quatre clignotants automatiques, X = détresse), **allumé de jour comme de nuit** avec un boîtier sombre par optique. **Toute mesure se fait dans le repère du quad** (`boiteDans`, `repereDe`) : sur une machine rapide le GLB arrive après le placement dans le village, et des mesures en coordonnées monde envoyaient pilote et feux à 400 m. Présenté au pilote comme une voiture (`construireEnginQuad`) ; réglages `REGLAGES.quad`. Depuis le 26/09 il **saute et fait les figures de la trottinette** (`SAUT_QUAD`, Espace/F/G) |
...
| Fichier | Rôle |
| --- | --- |
| `maps/voiture-physique.js` | La loi de conduite, sans Three.js : couple moteur, boîte 6 + marche arrière, transfert de charge, dérive des pneus, ellipse de friction, frein à main. C'est à la voiture ce que `flight-model.js` est au chasseur. **Mode arcade** (`ARCADE`, `arcade: true` sur gt / traction / quad, 26/09/2026) : on freine droit sans volant, marche arrière engagée en 0,4 s et couple ×1,6, couples et freins relevés |
...
| Fichier / asset | Rôle |
| --- | --- |
| `maps/poilhes-commerces.js` | L'épicerie **Ostal Louis** (devanture 3D sur sa façade, affiche au bord de la rue, pastille au-dessus des toits) et le **blason de l'Olympique Midi Lirou** devant le stade ; exporte `chercherFacade` |
| `maps/poilhes-mairie.js` | La **mairie** (26/09/2026) : tour de l'horloge qui traverse le toit (quatre cadrans, beffroi, cloche, flèche d'ardoise), entrée (porte, marches, balcon, plaque MAIRIE, devise, deux drapeaux, lanternes) posée sur la façade **qui regarde la place** (rayon tiré depuis la place vers le bâtiment, orientation par la normale du mur — les murs sont à une face, un rayon parti de l'intérieur touche la maison d'en face), et la **place** (pavage, fontaine, bancs, lampadaires) sur le premier disque libre autour de l'étiquette « Place de la Mairie ». Primitives + textures canvas, rien à télécharger |
...
| Fichier | Rôle |
| --- | --- |
| `maps/poilhes-skatepark.js` | Le parc entier : `profil(u, v)`, le maillage qui en découle, les plots **Refait le 25/09/2026** : plus rien de modelé à la main. Le parc = une dalle + des modules 3D (`assets/skatepark/*.glb`, lots fingerboard mis à l'échelle) posés d'après `maps/poilhes/skatepark-plan.json` ; la physique lit `skatepark-hauteurs.bin` (dessus des modules rastérisé au pas de 10 cm par `scripts/poilhes/skatepark_modeles.py hauteurs`). Changer le parc = éditer le plan, relancer `plan` (image) puis `hauteurs`. 26/09 : textures **générées** (béton, contreplaqué pour le lot FreeCAD, acier pour les rails) avec UV par boîte (`uvParBoite`, les GLB n'en ont pas) ; 29 pièces dont mini half-pipe, escalier à rail, quarters d'angle ; `relief_max` par pièce plafonne le relief rastérisé (un rail devenait un mur invisible) ; les GLB des nouvelles pièces sont copiés non compressés dans `assets/skatepark/` |
