...
class Pilot:
    ...
    last_fire_at: dict = field(default_factory=lambda: {"gun": 0.0, "missile": 0.0})
...
def quick_join(pilot=Depends(current_pilot)):
    ...
    room = next(
        (item for item in rooms.values() if len(set(item.reservations) | set(item.pilots)) < MAX_PLAYERS),
        None,
    )
    if room is None:
...
def finite_vector(value, size=3, limit=5000):
    ...
    try:
        ...
        return None
    return vector if all(math.isfinite(item) and abs(item) <= limit for item in vector) else None
...
async def release_disconnected_reservation(room: MatchRoom, user_id: int, disconnected_at: float):
    """Keep a seat briefly so a browser refresh can reconnect to the same duel."""
    await asyncio.sleep(15)
    if user_id in room.pilots or room.reservations.get(user_id) != disconnected_at:
        return
...
async def handle_fire(room: MatchRoom, attacker: Pilot, message: dict):
    ...
    if weapon not in {"gun", "missile"}:
        return
    cooldown = 1.2 if weapon == "missile" else 0.08
    damage = 100 if weapon == "missile" else 10
    now = time.monotonic()
    if now - attacker.last_fire_at[weapon] < cooldown:
        return
    attacker.last_fire_at[weapon] = now
    ...
    target = next((pilot for uid, pilot in room.pilots.items() if uid != attacker.user_id and pilot.hp > 0), None)
    ...
    max_range = 1400 if weapon == "missile" else 900
    if distance <= 0.01 or forward_length <= 0.01 or distance > max_range:
        return
    alignment = sum((delta[i] / distance) * (forward[i] / forward_length) for i in range(3))
    if alignment < (0.94 if weapon == "missile" else 0.985):
        return
    target.hp = max(0, target.hp - damage)
    await broadcast(
        room,
        {"type": "damage", "target_id": target.user_id, "attacker_id": attacker.user_id, "weapon": weapon, "hp": target.hp},
    )
    if target.hp == 0:
        await broadcast(room, {"type": "destroyed", "user_id": target.user_id, "by": attacker.user_id})
...
async def multiplayer_socket(websocket: WebSocket, room_id: str):
    ...
    await broadcast(room, {"type": "joined", "user_id": user_id, "username": pilot.username, "hp": pilot.hp}, exclude=user_id)
    try:
        while True:
            ...
            if message.get("type") == "state":
                ...
                await broadcast(room, {"type": "state", "user_id": user_id, "state": pilot.state, "hp": pilot.hp}, exclude=user_id)
        ...
        if room.pilots.get(user_id) is pilot:
            ...
            room.reservations[user_id] = disconnected_at
            await broadcast(room, {"type": "left", "user_id": user_id})
            asyncio.create_task(release_disconnected_reservation(room, user_id, disconnected_at))
        clean_rooms()
