 ...
 *
 * @param {object} o { world, root, solAt, url } — `url` = le route.json du pays
 * @returns {Promise<Array<THREE.Group>>} les portes, dans l'ordre
 */
export async function construireCourseRoute({ world, root, solAt, url }) {
  let points = null;
  if (url) {
    try {
  ...
  root.add(construireRuban(fin, solAt));
