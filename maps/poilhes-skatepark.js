...
export const DEPART = { x: CENTRE.x + 19, z: CENTRE.z + 2, cap: Math.PI };
...
function table(u, v) {
  ...
  else if (v < -1.9) h = H;                            // le plateau
  else h = H * lisse((1.6 - v) / 3.5);                 // la descente
  return h * large;
}

/** Les bosses : trois vagues de 3,2 m, la roue avant décolle sur chacune. */
function bosses(u, v) {
  const large = creneau(u, -23.2, -14.8, 1.0);
  if (large <= 0 || v < 4.5 || v > 14.1) return 0;
  return 0.42 * (1 - Math.cos(((v - 4.5) / 3.2) * Math.PI * 2)) / 2 * large;
}

/**
 * Le gros saut de la ligne droite sud : tremplin, trou de 4,5 m, réception.
 *
 * Le tremplin est **convexe** (exposant 1,7) : il se cabre en fin de course, ce
...
function saut(u, v) {
  ...
  if (large <= 0) return 0;
  let h = 0;
  if (u > -7.2 && u <= -2.2) h = 1.35 * Math.pow((u + 7.2) / 5, 1.7);
  else if (u >= 2.6 && u < 9.6) {
    // La réception : une face courte à passer — c'est le trou — puis une pente
    // qui descend dans le sens de la marche, pour absorber la chute.
    h = u < 3.6 ? 1.15 * lisse((u - 2.6) / 1.0) : 1.15 * lisse((9.6 - u) / 6.0);
  }
  return h * large;
}
...
function halfpipe(u, v) {
  const bout = creneau(u, -9.6, 9.6, 2.2);
  if (bout <= 0) return 0;
  const d = Math.abs(v - 3.0);
  const PLAT = 2.5, R = 5, MUR = 2.0, DECK = 1.4, DOS = 3.0;
  let h = 0;
  if (d <= PLAT) h = 0;
...
export function profil(u, v) {
  ...
  let h = dalle(u, v);
  const pieces = [virages(u, v), table(u, v), bosses(u, v), saut(u, v),
 ...
 * @returns {object} { root, hauteurAt, surParc, marquerAdherence, depart }
 */
export function construireSkatepark({ decor, centre = CENTRE }) {
  ...
  for (let j = 0; j < nv; j++) {
    for (let i = 0; i < nu; i++) {
      ...
      // La ligne de départ, en travers de la droite est.
      if (u > 14.2 && u < 23.8 && Math.abs(v - 2) < 0.45) teinte.copy(BLANC);
  ...
  return { root, hauteurAt, surParc, marquerAdherence, profil, centre: { x: cx, z: cz }, depart: DEPART };
}
